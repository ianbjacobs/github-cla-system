...
function stringList(value) {
  ...
  if (!Array.isArray(value)) throw new TypeError("labels must be an array.");
  return value
...
export function isAgreementPullRequest({
  ...
  return (
    stringList(labels).includes("agreement") &&
...
export function evaluateContributorAgreement(
  ...
  if (isAgreementPullRequest({ labels, headRef, headRepository, baseRepository })) {
    return {
      authorized: true,
      exempt: true,
